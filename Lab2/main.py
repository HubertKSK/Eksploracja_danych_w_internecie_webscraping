...
try:
    if os.path.exists("Lab2/export/content.csv"):
        open('Lab2/export/content.csv', 'w').close()
except Exception as e:
    pass


try:
    os.mkdir('Lab2/export')
...
def ngram(text_dump, n=1):
    text_dump = re.sub('[^A-Za-z0-9; ążźćńółęąśŻŹĆĄŚĘŁÓŃ]+', '', text_dump)
...
def extract_text(soup, webpage):
    try:
        ...
        text = text.strip()
        with open(f'Lab2/export/content.csv', 'a') as f:
            f.write(webpage + ';')
            f.write(text)
            f.write("\n")
...
def find_email(page_content):
    ...
    if emails:
        for idx, val in enumerate(emails):
            f = open('Lab2/export/emails.csv', 'a')
            write = csv.writer(f)
            write.writerow([val])
            f.close()
    return emails
...
def remove_duplicates(emails_file='Lab2/export/emails.csv'):
